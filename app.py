...
def addNewUser(username, password, cursor_object):
    try:
        ...
        cursor_object.execute("CREATE TABLE " + username + "( date varchar(30), time varchar(30), salat varchar(30), point float, state varchar(30), signer varchar(30), ID INT AUTO_INCREMENT, UNIQUE(ID, date, salat))")   
    except (mysql.connector.IntegrityError, mysql.connector.DataError) as err:
        print("DataError or IntegrityError")
        print(err)
        return FAIL
